...
"""
这套 Python 脚本是一个用于分析和提取 OpenFOAM 10 教程案例及其命令行工具信息的自动化工具，其目标是构建一个包含：
...
OpenFOAM 可执行命令及其 -help 帮助信息；

并统一保存为可供数据库构建或 AI 模型使用的纯文本格式。"""
import os
os.environ["CONFIG_FILE_PATH"] = "/mnt/d/Ubantu_run/MetaOpenFOAM/inputs/2D_detonation.yaml"  # 示例路径
import subprocess
import config_path
import gzip
...
def find_cases(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        if 'system' in dirs:
            ...
            folder_path = "/home/xupan/detonationFoam-main/applications/solvers/detonationFoam"  # 替换为你的文件夹路径
            # 只保留是文件夹的
            folder_names_new = [name for name in os.listdir(folder_path)
                            if os.path.isdir(os.path.join(folder_path, name))]
            print(folder_names_new)
            # 筛选出包含关键词的文件夹名
            matched_folders = [name for name in folder_names_new if name in allrun_content]
            if matched_folders:
                solver = matched_folders[0]
            # 添加找到的信息到cases列表
            cases.append({
                'case_name': case_name,
                'solver': solver,
...
tutorial_path = f"/home/xupan/detonationFoam-main/tutorials"
