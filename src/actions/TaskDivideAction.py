...
"""
    ...
    async def run(self, with_messages:List[Message]=None, **kwargs) -> List[str]:
        ...
        async_qa_tutorial_name = AsyncQA_tutorial_name()
        ...
        if json_str:
            ...
            sys.exit()
        # 根据各个任务的字符串值判断tasks的值          simulation>postprocessing>analysis(拿到独立变量)>optimization
        ...
        rsp = await async_qa_tutorial_name.ask(prompt_Translate)
        ...
        rsp = await async_qa_tutorial_name.ask(prompt_Find)          #这块似乎不需要调用模型
        doc = rsp["source_documents"]
        rsp2 = await async_qa.ask(f"用户需求的算例信息如下：<{user_case}>,文档列表如下：<{doc}>。请你从文档列表中选出最相似的算例，名称优先，其次看求解器。要求回答最匹配的算例的列表索引，不要出现其他多余文字。你的回答：")
        tutorial = doc[int(rsp2)]
        print('find_case',tutorial)

        if config_path.tasks>=3:
            prompt_ParaStudy = self.PROMPT_TEMPLATE_ParaStudy_Target.format(requirement=CFD_analysis_task) #找自变量和因变量
            ...
            config_path.Para_PATH = f"{config_path.Run_PATH}/{case_name}_paras"    #带不同参数的算例
            ...
            if os.path.exists(optparameter_path) and os.path.isfile(optparameter_path):
                ...
                save_json_path = f"{config_path.Para_PATH}/CFD_tasks_all.json"   #保存任务信息
                ...
                save_path = config_path.Para_PATH                                #保存仿真相似案例
                self.save_find_tutorial(tutorial.page_content, save_path)

                document_text = self.read_openfoam_tutorials(f"{config_path.Database_PATH}/openfoam_tutorials.txt")    #？？？？？看不出有啥用
                ...
                rsp = await async_qa.ask(promt_find_input)            #从相似案例中取出各自变量的具体数值
