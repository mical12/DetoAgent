...
class ReviewerAction(Action):
    ...
    The corresponding input file list is:
    {file_list} in folder {folder_list}
    Please analyze which file the error may be related to, and return the most related file and the corresponding folder in the following JSON format:
    ```
    {json_structure}
    ```with NO other texts.
    where file_name1, file_name2, ..., come from {file_list}
    ...
    async def run(self, with_messages:List[Message]=None, **kwargs) -> Message:
        ...
        if command != "None":
            ...
            if global_statistics.Executability < 3:
                ...
                """
                prompt3 = self.PROMPT_TEMPLATE_file_structure.format(command= command, error=error_content, file_names = files_names, file_folders = folder_names, json_structure = json_structure) #判断是不是丢失了一些关键文件
