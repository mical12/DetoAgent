...
为系统增加环境变量：
config_file_path
调用大模型Api的key和base_url

更新MetaGPT的配置文件config2.yaml，设置大模型的API类型、模型名称、代理和API密钥等信息。
"""
# input information
import os
import sys
...
searchdocs = config.get('searchdocs', 4)
...
#多模态输入部分
os.environ["BLOCKMESHDICT_PATH"] = str(config.get("BLOCKMESHDICT_PATH") or "")
os.environ["SETFIELDS_PATH"] = str(config.get("SETFIELDS_PATH") or "")
